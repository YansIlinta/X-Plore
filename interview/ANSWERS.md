...
**Q05**  
环境如 AutoDL 大核；指标 E2E P50/P90、错误、丢弃；低扇出 10K/1000 房 P50 1.6ms P90 5.3ms 错误 0。前提：压测机与业务分机、注意工具精度（纳秒透传）。`loadtest/` + `PROJECT.md` §5。
...
**Q11**  
通常 readPump + writePump 两 goroutine：读解包上行，写唯一出口保证无并发写 conn。`core/client.go`。
...
**Q14**  
query token：① 等于 `DANMU_AUTH_TOKEN`（压测）；② HS256 JWT，`uid` claim 强制覆盖。`comet/main.go` + `core/jwtauth.go`。
...
**Q21**  
256 平衡锁数量与 map 粒度；分片键 roomID hash（fnv）。`core/hub.go`。
...
**Q25–Q26**  
令牌与时间打包进一个 uint64，单次 CAS 更新，避免拆字段 ABA。`core/ratelimit.go`。
...
**Q35**  
Logic 或 standalone 本地 AC 过滤；多模式匹配高效。`core/filter.go`。
...
- 代码：`server/redis.go`；文档：`PROJECT.md` §3.1、§4.3。
...
**Q68**  
HTTP 注册+心跳 TTL；过期剔除。`minirpc/registry`。

**Q69**  
roomID 哈希选 Logic 实例，便于房间级限流。`logicpool`。

**Q70**  
失败计数→open→半开探测→关闭；保护下游。`minirpc/breaker`。
...
**Q75**  
连接数、消息 in/out、广播延迟、队列长度等。`core/metrics.go`。

...
if (-not (Test-Path (Join-Path $Root "EVOLUTION.md"))) { $Root = "D:\X-Plore\wavehub" }
...
foreach ($d in @("wavehub\EVOLUTION.md", "wavehub\ROADMAP-V2.md", "wavehub\deploy\platform\PRODUCTION.md", "docs\DEMO.md", "danmu\PROJECT.md")) {
  if (Test-Path (Join-Path $RepoRoot $d)) { Ok $d } else { Fail "$d missing" }
}

Write-Host ""
Write-Host "Next: see docs\DEMO.md for full startup & demo script." -ForegroundColor Cyan

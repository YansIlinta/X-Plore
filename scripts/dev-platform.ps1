...
function Show-Deps {
  ...
  foreach ($b in @(
      ...
      "wavehub-micro\bin\social.exe",
      "wavehub-micro\bin\search.exe",
...
function Build-Bins {
  $micro = Join-Path $Root "wavehub-micro"
  Set-Location $micro
  ...
  Set-Location $Root
  go build -o wavehub-micro/bin/comet.exe ./comet
  Write-Host "done -> wavehub-micro\bin\" -ForegroundColor Green
}
...
function Show-Help {
  ...
  Write-Host "    # danmu comet (repo root):"

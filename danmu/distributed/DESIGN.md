...
> 服务间通信用 **gRPC**（契约见 `pb/danmu.proto`），服务发现用自研的 `registry/`（HTTP + 内存 + TTL 租约），
> comet 选 logic 实例用自研的 `lb/`（一致性哈希环 + 虚拟节点），
...
| discovery(eureka) | `registry/` | HTTP + 内存 map + TTL 租约的注册中心 |
...
- 生产：registry 换 etcd/consul；Comet 连接层可换 epoll 事件循环上百万单机（见 REVIEW.md 演进）。

...
```text
Browser ──► ops (:7900)
              ├── GET registry /services            → 服务发现与实例清单
...
```bash
# 本地全链路 + console（需 Kafka 在 localhost:9092；registry:7350 logic:7410 comet×2 ops:7900）
...
#   bin/registry -addr=:17350
#   bin/logic -addr=:17400 -http-addr=:17410 -registry=http://localhost:17350 \
...
#     -id=comet1 -registry=http://localhost:17350 -trace-sample=1
#   bin/ops -addr=:17900 -registry=http://localhost:17350 -kafka=localhost:9092 -poll=1s
...
| 能力 | 来源 | 状态 |
|------|------|------|
| registry `/services` 服务发现 | 项目已有 | 直接复用 |
...
| 场景 | 结果 |
|------|------|
| A ops 发现 registry/logic/comet | ✓ services/topology 正确 |
...
- 观测代码必须 O(1)/聚合/有界：禁止为 Dashboard 遍历全部连接；实时消息、事件、trace
  全部是带上限的环形缓冲。
- ops 不依赖任何业务服务才能启动；registry 掉线用上次清单继续探测并在事件流里报警。

...
>
> **2026-08-16 二次更新（etcd 替换）**：自研 `registry/`（HTTP+内存+TTL）与 `lb/`（一致性哈希环）已整体替换为 **etcd**（注册=租约 keepalive，发现=前缀 Get/Watch）与 **gRPC 标准 round_robin**（经 etcd 官方 naming/resolver）。`cmd/registry` 随注册中心一并删除，新增 `etcdreg/` 封装注册/发现约定。涉及第 1/2/3/6/10/11/12/13/14 节。

## 1. 组件与端口

| 组件 | 源码 | 默认端口 | 注册到 etcd | 备注 |
|------|------|----------|------------|------|
| etcd | 标准组件（compose `quay.io/coreos/etcd:v3.5.21`） | `:2379`（客户端） | — | key 前缀 `danmu/services/`，10s 租约 + keepalive |
...
compose 内实际部署（`docker-compose.goim.yml`）：kafka、clickhouse、etcd、logic（宿主 7410）、job（宿主 7420）、comet1（宿主 8080）、comet2（宿主 8081）、consumer、nginx（宿主 8088）、✅ ops（宿主 7900）。`Dockerfile.goim` 编出 logic/job/comet/ops 四个二进制（registry 已随自研注册中心删除）。

**`*-http` 注册名约定**：业务服务把 RPC 地址注册为 `comet`/`logic`（供链路发现），把 HTTP 观测地址额外注册为 `comet-http`/`logic-http`/`job-http`（仅供 Ops Console 发现）。两者分开注册，观测面的增删不会污染消息链路的服务发现。注册 key 为 `danmu/services/<service>/<addr>`，value 是 naming.Update 兼容 JSON（`etcdreg/etcdreg.go:45` `Register`）。地址推导统一走各自的 `advertiseHTTPAddr()`（`comet/main.go:430`、`logic/main.go:240`、`job/main.go:370`）：显式 `-advertise-http` 优先，否则取 advertise 主机名 + HTTP 端口，advertise 为空则主机名用 `localhost`。

⚠️ **job 必须显式给 `-advertise-http`**：它没有 `-advertise` 参数，缺省推导只能得到 `localhost:7420`，注册进 etcd 后 ops 会拨到自己容器的 localhost。comet/logic 因为有 `-advertise`，缺省推导恰好正确，但 compose 里仍写全以免日后改 advertise 时静默失效。
...
>
> ✅ 2026-08-16 二次更新：`registry/` 与 `lb/` 这两个顶层包也已删除——服务发现换成 **etcd**（客户端 `go.etcd.io/etcd/client/v3` v3.7.1），comet→logic 的路由换成 **etcd naming/resolver + gRPC round_robin**（`comet/logicpool.go:25`）。新增 `etcdreg/` 包封装本项目在 etcd 上的注册/发现约定。
...
### etcd 注册模型（`etcdreg/etcdreg.go`，替代原自研 registry）

| 操作 | 说明 |
|------|------|
| 注册=心跳 | `Register(ctx, cli, service, addr, ttl)`：Grant lease → Put key → 持续 KeepAlive（`etcdreg/etcdreg.go:45`）。崩溃后 lease 到期 key 自动消失，**仍然没有注销接口**（有意设计，理由同原 registry） |
| key/value | key `danmu/services/<service>/<addr>`；value `{"Op":0,"Addr":"<addr>","Metadata":null}`（grpc naming.Update 兼容 JSON）——comet→logic 的官方 naming/resolver 与 ops 的 `ListAll` 在同一份数据上并存读取 |
| `List(ctx, service)` | 按前缀 Get 取存活地址（key 最后一段），排序输出 |
| `ListAll(ctx)` | 全部服务 map `{service: [addr...]}`——Ops Console 枚举拓扑的入口（对应原 `GET /services` 无参） |
| `Watch(ctx, service, onUpdate)` | 先回调一次当前列表，此后每次增删重读全量回调；job 用它维护 comet 连接池（替代原 3s 轮询） |

有测试覆盖：`etcdreg/etcdreg_test.go`（基于进程内 embed etcd：注册/发现/ctx 取消主动 Revoke/Watch/与官方 naming/endpoints 的 value 兼容性）。
...
现成四步链路自检：etcd 发现（`-etcd`，默认 `localhost:17379`）→ Logic.OnMessage → PushRoom→WS 下行 → 上行后抓 comet `/metrics` 验证计数递增。退出码非零=失败。可直接作为 Ops Console「链路自检」功能的后端。

## 11. 日志

全标准库 `log`，带 `[comet]/[logic]/[job]/[ops]/[etcdreg]/[kafka]` 前缀，无结构化、无级别。成功路径**零逐条日志**，无 OpenTelemetry。
...
### [existing] 可直接消费
- etcd `danmu/services/` 前缀（`etcdreg.List`/`ListAll`）→ 拓扑/实例清单数据源
...
| 2 | registry 补 `/health`；`/services` 无参列全部 | ✅ 已实现，**随后随 registry 一并移除** | 原实现在 `registry/registry.go:40-43`、`registry/registry.go:70-90`。2026-08-16 二次更新后自研 registry 整体替换为 etcd，等价能力由 `etcdreg.ListAll`（全量 map）与 etcd 自身活性探测承担 |
...
**默认参数**（`cmd/ops/main.go:22-29`）：`-addr :7900`、`-etcd localhost:2379`、`-kafka localhost:9092`、`-poll 2s`、`-mock false`。`-token` 为空时依次回退 `DANMU_AUTH_TOKEN` → 硬编码 `danmu-secret-token`（回退时打 WARN 日志）。
...
1. ~~**compose 未纳入 ops**~~ —— ✅ 2026-08-16 已解决：`Dockerfile.goim` 加编 ops 二进制，compose 加 ops 服务（宿主 7900）、映射 logic `:7410` / job `:7420`、四个业务服务补齐 `-advertise-http`。**尚未在真机容器里跑通**（当时 Docker daemon 未启动，只做了 `docker compose config` 语法校验）；等价接线已用宿主进程验证。2026-08-16 二次更新：compose 里的 registry 服务换成 etcd（`quay.io/coreos/etcd:v3.5.21`），宿主进程等价接线也已复验（见 §16）。首次 `docker compose up` 时仍需确认一次。
...
6. **`/health` 全是静态的**：comet/logic/job 三家的 `/health` 都不检查依赖，只证明进程还在（registry 已删除；etcd 的活性由 ops 侧 `etcd_up` 间接反映）。这次排查 trace 时被它坑了一下：Kafka 不可达、900 条消息全部写失败，三个 `/health` 依然全绿。
...
## 16. etcd 替换验证状态（2026-08-16 二次更新）

自研 `registry/`/`lb/`/`cmd/registry/` 删除后，以真实 etcd（v3.5.21 官方二进制）+ 宿主进程做了等价复验：

- **单测**：`etcdreg/etcdreg_test.go`（embed etcd：注册/发现/Revoke/Watch/官方 naming 兼容）、`comet/logicpool_test.go`（embed etcd + 2 个 mock logic，round_robin 真实分摊 20 个并发上行）、`ops/collector_test.go` 新增 etcd 发现注入两条用例。
- **chaintest 全过**（`-etcd` 指向真 etcd）：etcd 发现 → `Logic.OnMessage` → `PushRoom→WS` 下行 → 上行计数，exit 0。
- **job watch 实测**：第二台 comet 上线后 job `comets[]` 1→2，无需重启。
- **ops 实测**：2 台 comet 时 services/topology 正确；顺带修复 §13 采集链路的既有 bug——`probeServices` 的组件去重循环读的是空 map，同组件 ≥2 实例时整组重复（compose 恰好 2 台 comet，必触发），已加回归测试。
- **standalone comet 复验**：无 etcd 时 loadtest 200 连接零错误。
- **仍未实测**：`logic→Kafka→job 消费` 段（本会话无 Kafka broker，与 §15 一致）。

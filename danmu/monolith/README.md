...
> 这是**单体架构**版本：一个 `server` 进程同时承担接入层 + 业务逻辑 + 跨机广播。
> 同一系统的**分布式（goim 式 Comet/Logic/Job）版本**在 [`../distributed/`](../distributed/)，
> 两版并存互为对照，架构取舍见 [`../PROJECT.md`](../PROJECT.md)。
> 本目录的代码审查结论见 [REVIEW.md](./REVIEW.md)。
>
> `consumer/`（Kafka→ClickHouse 落库）与 `loadtest/`（HDR 压测客户端）是**架构无关的公共组件**，
> 源码放在本目录，分布式版也直接复用。
...
```
danmu/monolith/          # module github.com/YansIlinta/danmu-monolith
├── server/              # 弹幕服务器

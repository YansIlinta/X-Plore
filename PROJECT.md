...
| 形态 | 目录 | 跨机广播 | 适用场景 |
|------|------|----------|----------|
| **单体基线** | `server/` | Redis Pub/Sub | 开发/面试讲清「长连接 + 扇出」；少依赖快速演示 |
| **goim 微服务** | `comet/` + `logic/` + `job/` + `core/` + `minirpc/` | Kafka → Job → Comet.PushRoom | 连接/逻辑/扇出独立扩缩；对齐 Bilibili goim 思路 |
...
- 落库：`consumer/` 独立消费同一 Kafka topic 写 ClickHouse，与广播解耦。
...
| 组件 | 路径 | 职责 |
|------|------|------|
| **server** | `server/` | 单体：WS + Hub + Worker + Redis/Kafka + REST API |
| **core** | `core/` | 共享：Hub 分片、Client、消息、限流、敏感词、令牌、指标 |
| **comet** | `comet/` | 连接层：WS、房间连接、上行转发 Logic、暴露 PushRoom |
| **logic** | `logic/` | 无状态逻辑：过滤、msg_id、Kafka produce |
| **job** | `job/` | 消费 Kafka，发现全部 Comet 并定向 PushRoom |
| **registry** | `cmd/registry/` + `minirpc/registry` | HTTP 注册中心（TTL 租约） |
| **minirpc** | `minirpc/` | 自研 RPC：服务发现、一致性哈希 LB、熔断 |
| **consumer** | `consumer/` | Kafka → ClickHouse 落库 |
| **loadtest** | `loadtest/` | 多连接压测、E2E 延迟（纳秒透传）、HDR 报告 |
| **web** | `web/` | 前端（虚拟滚动弹幕列表） |
| **pb** | `pb/` | gRPC/Protobuf 契约（Comet 等） |
...
goim 链路：`cmd/chaintest` 已验证 registry 发现、Logic.OnMessage、Job→Comet.PushRoom→WS 收包；全链路含 Kafka 可用 `scripts/run-goim-local.sh` 或 `docker-compose.goim.yml`。
...
详见 **[EVOLUTION.md](./EVOLUTION.md)**。弹幕主线（本文 1–10 节）仍为连接层与压测叙事核心，不被视频业务替换。

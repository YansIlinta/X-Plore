...
| Q03 | 你的角色 | 从 0 到 1 你独立做了哪些、哪些复用生态 | 目录 `danmu/monolith/server/` `danmu/distributed/comet/` `danmu/distributed/minirpc/` | 主要职责 |
| Q04 | 最大难点 | 挑一个最难问题讲 STAR（建议：高扇出/锁/双路径） | `REVIEW.md` | 建连/广播/演进 |
| Q05 | 量化成果 | 说出你压测的**环境与数字**，以及数字的可信条件 | `PROJECT.md` §5、`danmu/monolith/loadtest/` | 成果 P50/P90 |
| Q06 | 为什么可丢 | 弹幕为什么允许丢消息？和 IM 有何不同 | `danmu/distributed/core/client.go` sendCh | 设计取舍 |
...
| Q10 | 技术选型 | 为什么 Go + WebSocket 而不是 SSE/长轮询 | `danmu/monolith/server/` `danmu/distributed/comet/` | 技术栈 |
...
| # | 网上问法 | 题目 | 代码/文档 | 简历 |
|---|----------|------|-----------|------|
| Q11 | 连接模型 | 一条连接几个 goroutine？职责划分？ | `danmu/distributed/core/client.go` ReadPump/WritePump | 连接模型 |
| Q12 | 背压 | sendCh 满了怎么办？为什么不阻塞？ | `danmu/distributed/core/client.go` TrySend | 慢客户端保护 |
| Q13 | 心跳 | 如何检测死连接？会话过期怎么断？ | `danmu/distributed/core/client.go` WritePump、session | 会话令牌 |
| Q14 | 鉴权 | WS 握手如何鉴权？JWT 与 secret 双模？ | `danmu/distributed/comet/main.go` handleWebSocket | 安全 |
| Q15 | Origin | 生产 CheckOrigin 应如何配置？ | `danmu/distributed/comet/main.go` makeCheckOrigin | 生产硬化 |
...
| Q19 | 房间进出 | 用户进房/退房如何注册到 Hub？并发安全吗？ | `danmu/distributed/core/hub.go` AddClient/RemoveClient | 分片 Hub |
| Q20 | 广播路径 | 一条弹幕从读到写到多个连接的完整路径（单体） | `danmu/monolith/server/worker.go` `hub` | 批量广播 |
...
| # | 网上问法 | 题目 | 代码/文档 | 简历 |
|---|----------|------|-----------|------|
| Q21 | 分片锁 | 为什么 256 分片？如何选分片键？ | `danmu/distributed/core/hub.go` numShards | 256 分片 |
| Q22 | 全局锁危害 | 全局 RWMutex 在万级连接下会发生什么？ | `REVIEW.md`、hub 演进 | 锁竞争 STAR |
| Q23 | Close 竞态 | Client.Close 有哪些并发坑？如何修？ | `danmu/distributed/core/client.go`、`REVIEW.md` | Close 竞态 |
| Q24 | channel 关闭 | 谁关闭 sendCh？双重 close 如何避免？ | `danmu/distributed/core/client.go` | 正确性 |
| Q25 | 令牌桶 | 无锁令牌桶如何实现？ABA 是什么？ | `danmu/distributed/core/ratelimit.go` | 令牌桶 ABA |
| Q26 | CAS | 为什么用单次 CAS 打包 tokens+时间？ | `danmu/distributed/core/ratelimit.go` packState | 令牌桶 |
| Q27 | 对象池 | Message 是否用了 sync.Pool？何时该用 | `danmu/distributed/core/message.go` | 工程优化 |
| Q28 | context | 优雅关闭如何用 context 传播？ | `danmu/distributed/comet/main.go` signal | 优雅关闭 |
...
| Q32 | 批量 | 按条数与时间窗批量的利弊？窗口如何选？ | `danmu/monolith/server/worker.go` | 10ms 窗口 |
| Q33 | syscall | 为什么批量能降延迟的 P90？ | 广播扇出 | 写放大 STAR |
| Q34 | 本地广播 | standalone comet 本地广播路径 | `comet` localBroadcast | goim 冒烟 |
| Q35 | 过滤 | 敏感词在哪一层做？AC 自动机复杂度 | `danmu/distributed/core/filter.go` | 敏感词 |
| Q36 | msg_id | 全局 msg_id 谁生成？格式？ | `danmu/distributed/logic/main.go` nextMsgID | 去重 |
...
| Q41 | 消费组 | storage 与 broadcast 消费组如何隔离 | `danmu/monolith/consumer/main.go` | 多消费组 |
...
| Q45 | 削峰填谷 | Job 消费 Kafka 再推 Comet 解决什么问题 | `danmu/distributed/job/` | goim 扇出 |
...
| # | 网上问法 | 题目 | 代码/文档 | 简历 |
|---|----------|------|-----------|------|
| Q49 | 为何 CH | 为什么历史用 ClickHouse 不用 MySQL？ | `danmu/monolith/consumer/db.go` | MergeTree |
| Q50 | 表设计 | 分区键、排序键如何设计？查询模式？ | danmu_history ORDER BY | 按天分区 |
| Q51 | 批量写 | 为何 batch insert？小批量有何问题？ | BatchInsert | 吞吐 |
| Q52 | 历史 API | `/history` 曾是死接口，根因与修复？ | `danmu/monolith/server/history.go` REVIEW H2 | 审查 |
...
| Q57 | PushRoom | Job 如何推？无房间为何 delivered=0 | `danmu/distributed/pb/danmu.proto` PushRoom | 定向推 |
| Q58 | 为何不本地 echo | 发送者消息也走回路的原因 | DESIGN-goim | 一致性 |
| Q59 | 无状态 | Logic 为什么无状态？如何扩 | `danmu/distributed/logic/main.go` | 水平扩展 |
...
| # | 网上问法 | 题目 | 代码/文档 | 简历 |
|---|----------|------|-----------|------|
| Q67 | 为何自研 | 为什么自研 minirpc 而不是直接 gRPC？ | `danmu/distributed/minirpc/` | 自研 RPC |
| Q68 | 服务发现 | registry 如何实现？TTL 租约？ | `cmd/registry`、`danmu/distributed/minirpc/registry` | 发现 |
| Q69 | 一致性哈希 | Logic 路由如何按 room 哈希？ | `danmu/distributed/minirpc/lb`、logicpool | 一致性哈希 |
| Q70 | 熔断 | 熔断器状态机？失败阈值？ | `danmu/distributed/minirpc/breaker` | 熔断 |
| Q71 | 生产替换 | 生产用 etcd/K8s DNS 怎么演进 | DESIGN 边界 | 后置 |
| Q72 | gRPC vs HTTP | 对内 gRPC 对外 HTTP 的理由 | Kratos proto-first | 技术栈 |
| Q73 | Proto | OnMessage 请求字段有哪些？offset_ms 用途 | `danmu/distributed/pb/danmu.proto` | 协议 |
...
| # | 网上问法 | 题目 | 代码/文档 | 简历 |
|---|----------|------|-----------|------|
| Q75 | 指标 | 暴露了哪些 Prometheus 指标？ | `danmu/distributed/core/metrics.go` | Prometheus |
| Q76 | 高基数 | 为何 room_id 不能做 label？ | REVIEW H1 | 高基数修复 |
| Q77 | pprof | 线上如何用 pprof 定位？ | pprof 端口 | 调优 |
| Q78 | 压测工具 | loadtest 如何测 E2E？HDR 是什么？ | `danmu/monolith/loadtest/` | HDR |

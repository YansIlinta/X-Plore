...
> 决策日期：2026-07-17  
> 状态：**MVP 代码主线完成**（M1–M6 + 前端体验）；真机转码依赖 Docker/ffmpeg。演示见 [DEMO.md](./DEMO.md)。  
...
| 业务底座 | `wavehub-micro`（Kratos + gRPC） |
...
| 路径 | 角色 |
|------|------|
| `server/` | 弹幕**单体基线**，压测对照，保留 |
| `comet/` `logic/` `job/` `core/` `minirpc/` | 弹幕微服务，继续作为实时通道 |
| `wavehub/` | 音乐站 Gin 单体，学习对照，不作为视频主线 |
| `wavehub-micro/` | **视频平台业务主线**（user + video + media） |
| `web/index.html` | 弹幕联调页，保留 |
| `web-app/` | **产品前端**（React） |
...
# 3. 网关（可选但推荐）
cd wavehub-micro && go run ./app/gateway/cmd
